...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    window.db.collection('config').doc('main').get().then(docSnap => {
        if (docSnap.exists) {
            ...
            if(data.texts) {
                ...
                if(data.texts.date) {
                    document.getElementById('d-date').textContent = data.texts.date;
                    const dTrailer = document.getElementById('d-date-trailer');
                    if(dTrailer) dTrailer.textContent = data.texts.date;
                }
    ...
    // Trailer Intro & Hero Animation
    const trailerIntro = document.getElementById('trailerIntro');
    const scene1 = document.querySelector('.trailer-scene--1');
    const scene2 = document.querySelector('.trailer-scene--2');
    const reducedMotion = window.matchMedia('(prefers-reduced-motion: reduce)').matches;

    // We still want GSAP to animate the hero in after the trailer
    const triggerHero = () => {
        gsap.to(".hero-anim", { 
            opacity: 1, 
            y: 0, 
    ...
    if (reducedMotion) {
        setTimeout(() => { if (trailerIntro) trailerIntro.classList.add('fade-out'); }, 500);
        setTimeout(() => { if (trailerIntro) trailerIntro.classList.add('hidden'); }, 1200);
        setTimeout(triggerHero, 600);
    } else if (trailerIntro) {
        // letterbox bars slide in
        setTimeout(() => trailerIntro.classList.add('bars-in'), 50);

        // scene 1: "Sisto compie 18 Anni"
        setTimeout(() => scene1.classList.add('active'), 400);
        setTimeout(() => trailerIntro.classList.add('flash'), 400);

        // letterbox opens and the site is revealed
        setTimeout(() => trailerIntro.classList.remove('bars-in'), 2800);
        setTimeout(() => trailerIntro.classList.add('bars-out'), 2800);
        setTimeout(() => trailerIntro.classList.add('fade-out'), 3000);
        setTimeout(() => { trailerIntro.classList.add('hidden'); triggerHero(); }, 3900);
    } else {
        triggerHero();
    }
    ...
    if (window.matchMedia("(pointer: fine)").matches) {
        ...
        ctaBtns.forEach(btn => {
            const wrap = btn.closest('.cta-btn-wrap');
            if(wrap) {
                btn.addEventListener('mousemove', (e) => {
                    ...
                    const dx = (x - cx) / cx;
                    const dy = (y - cy) / cy;
                    wrap.style.transform = `translate(${dx * 5}px, ${dy * 5}px)`;
                });
    ...
    // Countdown Logic
    let countDownDateString = "Sep 24, 2026 20:00:00"; // default
    if (window.firebaseData && window.firebaseData.texts && window.firebaseData.texts.countdown) {
        // datetime-local is YYYY-MM-DDTHH:MM
        countDownDateString = window.firebaseData.texts.countdown;
    }
    ...
    if (starsCanvas && !window.matchMedia("(prefers-reduced-motion: reduce)").matches) {
        ...
        function animateStarsCanvas() {
            // Trail effect (semi-transparent clear based on bg color)
            let trailColor = 'rgba(3, 3, 8, 0.4)';
            if (window.firebaseData && window.firebaseData.colors && window.firebaseData.colors.bg) {
                trailColor = hexToRgba(window.firebaseData.colors.bg, 0.4);
            }
