...
colorFields.forEach(field => {
    ...
    if (colorPicker && textInput) {
        colorPicker.addEventListener('input', (e) => {
            ...
            if(field === 'bg') document.body.style.backgroundColor = e.target.value;
        });
        textInput.addEventListener('input', (e) => {
            ...
            if (/^#[0-9A-F]{6}$/i.test(val)) {
                ...
                if(field === 'bg') document.body.style.backgroundColor = val;
            }
        });
    }
});
...
settingsForm.addEventListener('submit', async (e) => {
    ...
    let bgToSave = document.getElementById('color-bg-text').value || document.getElementById('color-bg').value;
    if (bgToSave && !bgToSave.startsWith('#') && bgToSave.length === 6 && /^[0-9A-F]{6}$/i.test(bgToSave)) {
        bgToSave = '#' + bgToSave;
    }
    ...
    try {
        ...
        alert('ERRORE! Le modifiche non sono state salvate nel database. Motivo: ' + error.message);
    } finally {
...
// Button sheen effect for admin panel
document.addEventListener('mousemove', (e) => {
    ...
    ctaBtns.forEach(btn => {
        ...
        if(wrap) {
            ...
            // Check if mouse is hovering over the button wrap
